...
public class AuthController {
    ...
    public String signup(Model model) {
                          ...
                          Model model)
     {
         ...
         //getRequestURL()メソッドを使って、リクエストURL(https://ドメイン名/signup)を取得する。
         String requestUrl = new String(httpServletRequest.getRequestURL());
         //このイベントの実行で、会員登録のタイミングでイベントを発行することが出来る。
         //引数にはUserのオブジェクトとリクエストURLを渡す。
         signupEventPublisher.publishSignupEvent(createdUser, requestUrl);
         redirectAttributes.addFlashAttribute("successMessage","ご入力いただいたメールアドレスに認証メールを送信いたしました。");
